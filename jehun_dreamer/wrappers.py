...
class RaceCarBaseEnv:
    def __init__(self, track, task, rendering=False):
        ...
        if env_id not in envs.keys():
            scenario = f"scenarios/{task}/{track}.yml"
...
class RaceCarWrapper:
    ...
    def step(self, actions):
        ...
        obs, reward, done, _, info = self._env.step(actions)
    ...
    def reset(self, **kwargs):
        ...
        for agent_id in self.agent_ids:
            ...
            if 'low_res_camera' in obs[agent_id]:
                obs[agent_id]['iamge'] = obs[agent_id]['low_res_camera']
        return obs
    
    def render(self, **kwargs):
        if kwargs:
            raise TypeError("render() does not need keyword arguments anymore in gymnasium env") # [DEBUG]

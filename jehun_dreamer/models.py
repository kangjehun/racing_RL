...
class Dreamer(tools.Module):
    ...
    def __init__(self, config, datadir, actspace, obsspace, writer):
        self._c = config
        self._actspace = actspace['A']
        self._obsspace = obsspace['A']
    ...
    def __call__(self, obs, reset, state=None, training=True):
        ...
        if state is not None and reset.any():
            # [DEBUG] Never reached during training, since reset is always False
    ...
    def _build_model(self):
        acts = dict(
            elu=tf.nn.elu, relu=tf.nn.relu, swish=tf.nn.swish, leaky_relu=tf.nn.leaky_relu)
        ...
        if self._c.obs_type == 'image':
            ...
            self._decode = LidarDistanceDecoder(128, self._obsspace['lidar'].shape)
    ...
    def _train(self, data, log_images):
        ...
        with tf.GradientTape() as actor_tape:
            ...
            returns = tools.lambda_return( # V(s)
        ...
        with tf.GradientTape() as value_tape:
            value_pred = self._value(imag_feat)[:-1] # v(s)
    ...
    def _imagine_ahead(self, post):
        ...
        return imag_feat # (Horizon, Batch * (Sequence Length -1), stoch_size + deter_size) = (15, 450, 230)
    ...
    def _image_summaries(self, data, embed, image_pred):
        summary_size = 6  # number of images to display (batches)
        summary_length = 5  # number of sequence length
        if self._c.obs_type in ['image', 'lidar']:
            ...
            # 5 closed-loop predictions (images) + 45 steps of open-loop predictions 
            ...
            recon = image_pred.mode()[:summary_size]
            recon = tf.cast(recon, tf.float32)  # concatenation requires the same dtype
            init, _ = self._dynamics.observe(embed[:summary_size, :summary_length],
                                             data['action'][:summary_size, :summary_length])
            init = {k: v[:, -1] for k, v in init.items()}
            prior = self._dynamics.imagine(data['action'][:summary_size, summary_length:], init)
            ...
            model = tf.concat([recon[:, :summary_length], openl], 1) # recon/openl is already binary, no need scaling
            error = (model - truth + 1) / 2
            openl = tf.concat([truth, model, error], 2)
    ...
    def policy(self, obs, state, training):
        if state is None: 
            # [DEBUG] len(obs[self._c.obs_type]) seems not equal to the batch size...
            # However, since the dynamics is trained once at __init__, this part never runs
    ...
    def _exploration(self, action, training):
        if training:
            amount = self._c.expl_amount # 0.3
            if self._c.expl_decay: # 0.0
                amount *= 0.5 ** (tf.cast(self._step, tf.float32) / self._c.expl_decay)
            if self._c.expl_min: # 0.3
                amount = tf.maximum(self._c.expl_min, amount)
            self._metrics['expl_amount'].update_state(amount)
        elif self._c.eval_noise: # 0.0
        ...
        if self._c.expl == 'epsilon_greedy': # [DEBUG] one_hot may not be a good choice
...
class LidarDistanceDecoder(tools.Module):
    ...
    def __call__(self, features):
        # note: features = tf.concat([state['stoch'], state['deter']], -1)]) [DEBUG] Need to check this
